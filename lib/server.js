    ...
    return new Promise(function(resolve, reject) {
        ...
        try {
            switch (method) {
                ...
                case 'fundAccount':
                    console.log(self.server.network, self.server.faucet);
                    if (self.server.network !== 'morden' || !self.server.faucet) { throw errors.MethodNotFound; }
                    self.server.faucet.send(utils.ensureAddress(params.address), '0x2b992ddfa23249d6').then(function(txid) {
                        resolve(txid);
                    }, function(error) {
                        reject(errors.ServerError);
                    });
...
*/
    ...
    utils.defineProperty(this, 'faucet', (function() {
        if (options.faucetPrivateKey) {
            return new Faucet(options.faucetPrivateKey, self.web3)
        }
        return null;
    })());
