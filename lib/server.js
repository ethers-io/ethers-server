...
function Server(options) {
    ...
    this.on('web3Connect', function(url) {
        console.log('web3.connect(' + url + ')');
    });

    this.on('web3Error', function(error) {
        self.emit('debug', 'web3Error', error);
...
Server.prototype._connectFilter = function() {
}
    ...
    var retry = function () {
        self._web3.eth.getBlockNumber(function (error, blockNumber) {
            ...
            if (error) {
                ...
                // Try connecting to a new instance of a web3
                //self._web3.reset();
                //self._web3 = new Web3(getProvider(self._rpc));

                // @TOOD: re-install all filters
            ...
            if (!self._web3Connected) {
                ...
                self.emit('web3Connect', self._rpc);
            }
            ...
            // This sets up a watchdog to make sure we are connected
            var watchdogTimer = null;
            var resetWatchdogTimer = function() {
                if (watchdogTimer) { clearTimeout(watchdogTimer); }

                // After 30s without a block, try reconnecting
                watchdogTimer = setTimeout(function() {
                    self.emit('debug', 'long block time (over 30s)');
                    self._reconnectWeb3();
                }, 30000);
                watchdogTimer.unref();
            }
            ...
            // @TOOD: Add an error detection thing into web3 and watchdog it
        });
    }

    // Bootstrap the first retry immediately
...
Server.prototype.start = function(callback) {
    ...
    var countConnectionsTimer = setInterval(function() {
        ...
        self._server.socket.getConnections(function(error, count) {
            if (error) {
                self.emit('debug', 'getConnections errror', error);
                return;
            }
...
Server.prototype._handleClose = function(client, code, reason) {
    this.emit('debug', 'Closing ' + client.ethersCliendId + '...');
    ...
    switch (method) {
        ...
        case 'ethers_deployContract':
            ...
            if (transaction.to.length != 0) {
                return invalidParamaeter('has a to');
            }
            ...
            Promise.all([
                ...
                Contracts.compile(source, contractName, function (error, contract) {
                    if (error) {
                    ...
                    self._contracts.store(transaction, networkHashes[self._network], contract, function(error, info) {

                        if (error) {
                            self.emit('debug', new Error(error.message));
                            return invalidParameter('unknown network');
                        }

                        self._web3.eth.sendRawTransaction(signedTransaction, function(error, hash) {
                            if (error) {
                                self.emit('debug', 'sendRawTransaction', error);
                ...
                self.emit('debug', error.message, error);
                callback(errors.ServerError);
        ...
        case 'ethers_getContract':
            ...
            this._web3.eth.getCode(address, 'pending', function(error, bytecode) {
                ...
                self._contracts.fetch(address, networkHashes[self._network], bytecode, function(error, info) {
                    if (error) {
                        return callback(errors.ServerError);
                    }
...
Server.prototype._handleInstallFilter = function(client, filterId, filterInfo) {
...
Server.prototype._handleFilterMethod = function(client, method, params, callback) {
    ...
    try {
        ...
        //filter._ethersFilterInfo = filterInfo;
