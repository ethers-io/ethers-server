...
/*
Server.prototype._resetWatchdog = function(reschedule) {
    ...
    this._watchdogTimer = setTimeout(function() {
        self.emit('debug', 'long block time (over 30s) ' + ((new Date()).getTime() - self._t0));
        self._reconnectWeb3();
    }, 30000);
...
//    this._resetWatchdog(false);
    ...
    // Bootstrap the first retry immediately (but use timeout, so clearTimeout works)
