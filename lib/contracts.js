 ...
 *
 *  Idea 2
 *    - Map (multihash(source, SHA2-256) => (gistId, url))
 *    - Map ("A" + sha(address, bytecode, network) => (gistId, deploymentTarget, compilerVersion, optimized, url))
 *
...
function setupMethods (soljson){
    ...
    var compile = function(input, optimize, callback) {
        if (!optimize) { console.log('WARNING: optimize disabled'); }

        var result = null;
        if (callback !== undefined && compileJSONCallback !== null) {
            result = compileJSONCallback(JSON.stringify(input), optimize, function () {});
        } else if (typeof(input) != typeof('') && compileJSONMulti !== null) {
            result = compileJSONMulti(JSON.stringify(input), optimize);
        } else {
            result = compileJSON(input.sources['contract.sol'], optimize);
        }
...
function makeCompile(compilerVersion) {
    ...
    var compile = function(source, deploymentTarget, callback) {
        _compile({sources: {"contract.sol": source}}, optimize, function (compiled) {
            var errors = [], warnings = [];
            if (compiled.errors) {
...
var compile = makeCompile('v0.3.5-2016-06-27-2ccfea8');
...
GitHubAWSContracts.getSourceKey = function(source) {
    source = new Buffer(source, 'ascii');

    var hasher = crypto.createHash('sha256');
    hasher.update(source);
    var hash = hasher.digest()

    var multihash = new Buffer(2 + hash.length);
    multihash[0] = 0x12;
    multihash[1] = hash.length;
    hash.copy(multihash, 2);
...
GitHubAWSContracts.prototype.store = function(transaction, network, contract, callback) {
    ...
    var saveAddress = function(info) {
        self._setAttributes(addressKey, {
            ...
            optimized: (contract.optimized ? 'yes': 'no'),
            sourceKey: sourceKey,
    ...
    this._getAttributes(sourceKey, function(error, info) {
        ...
        self._github.gists.create({
            ...
            var info = {
                gistId: result.id,
                sourceUrl: result.files['contract.sol'].raw_url,
...
function Contracts(options) {
    ...
    Object.defineProperty(this, 'store', {
        value: function(transaction, network, contract, callback) {
            if (!utils.isHexString(network, 32)) {
    ...
    Object.defineProperty(this, 'fetch', {
        value: function(address, network, bytecode, callback) {
            ...
            if (!utils.isHexString(network, 32)) { throw new Error('invalid network'); }
            if (!utils.isHexString(bytecode)) { throw new Error('invalid bytecode'); }
...
//console.log(GitHubAWSContracts.getSourceKey('contract goo { }'));
